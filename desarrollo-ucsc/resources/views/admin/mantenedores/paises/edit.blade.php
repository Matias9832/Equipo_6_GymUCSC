...
@section('content')
    ...
    <form action="{{ route('paises.update', $pais->id_pais) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="bandera_pais" id="bandera_pais" class="form-control" value="{{ $pais->bandera_pais }}" required oninput="previewImage()">
        </div>
        <div class="mb-3">
            <label class="form-label">Vista previa de la bandera:</label>
            <div>
                <img id="bandera_preview" src="{{ $pais->bandera_pais }}" alt="Vista previa de la bandera" style="width: 100px; height: auto;">
            </div>
        </div>
