                        ...
                        <form action="{{ route('talleresnews.update', $newstalleres->id_noticia) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="nombre_noticia" class="form-control" placeholder="Título de la noticia"
                                    value="{{ old('nombre_noticia', $newstalleres->nombre_noticia) }}" required>
                            </div>
                            <div class="form-group">
                                <label for="descripcion_noticia">Cuerpo</label>
                                <textarea name="descripcion_noticia" class="form-control" placeholder="Cuerpo de la noticia" required>{{ old('descripcion_noticia', $newstalleres->descripcion_noticia) }}</textarea>
                            </div>
                                ...
                                <div class="row">
                                    <div class="col-6">
                                        <input type="text" class="form-control" id="featured_date" name="featured_date" placeholder="Fecha"
                                            autocomplete="off"
                                            value="{{ old('featured_date', $newstalleres->featured_until ? \Carbon\Carbon::parse($newstalleres->featured_until)->format('Y-m-d') : '') }}">
                                    </div>
                                    <div class="col-6">
                                        <input type="text" class="form-control" id="featured_time" name="featured_time" placeholder="Hora"
                                            autocomplete="off"
                                            value="{{ old('featured_time', $newstalleres->featured_until ? \Carbon\Carbon::parse($newstalleres->featured_until)->format('H:i') : '') }}">
                                    </div>
                                </div>
                            </div>
