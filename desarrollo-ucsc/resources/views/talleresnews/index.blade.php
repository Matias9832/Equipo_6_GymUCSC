            ...
            <div class="position-relative z-1 d-flex align-items-center h-100 ps-5 p-5">
                ...
                <div class="bg-primary bg-opacity-75 text-white p-md-5 rounded shadow position-relative" style="max-width: 650px;">
                    ...
                    @if(Auth::check() && Auth::user()->is_admin)
                        <a href="{{ route('talleressettings.edit') }}" 
            ...
            <div class="card shadow-sm text-center p-5 position-relative mb-2">
                ...
                @if(Auth::check() && Auth::user()->is_admin)
                    <div class="card-header pb-0 d-flex justify-content-between align-items-center mb-4">
                        <a href="{{ route('talleres.create', ['origen' => 'noticias']) }}" class="btn btn-primary position-absolute" style="top: 15px; right: 15px; z-index: 1;">
                            Crear nuevo taller
                        </a>
                    </div>
                @endif
                                    ...
                                    <div class="d-flex justify-content-between align-items-center px-3 py-2 bg-white border rounded-top" style="cursor: pointer;">
                                        ...
                                        @if(Auth::check() && Auth::user()->is_admin)
                                            <div class="d-flex gap-2 ms-3 pt-2">
                                                ...
                                                <form action="{{ route('talleres.destroy', ['taller' => $taller->id_taller, 'origen' => 'noticias']) }}"
                                                    method="POST"
                                        ...
                                        @if($taller->horarios && $taller->horarios->count())
                                            ...
                                            <ul>
                                                @foreach($taller->horarios as $horario)
                                                    <li>{{ $horario->dia_semana }} de {{ $horario->hora_inicio }} a {{ $horario->hora_termino }}</li>
                                                @endforeach
                                            </ul>
                                        @endif
                                    </div>
                        ...
                        <div class="carousel-item @if($index == 0) active @endif" style="background-color: white; border-radius: 1rem; padding: 1rem;">
                            <div class="row align-items-center">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <a href="{{ route('talleresnews.show', $noticia->id_noticia) }}" class="btn btn-primary btn-sm">Ver más</a>
                                </div>
                            </div>
                        </div>
                    @endforeach
                        ...
                        @if(Auth::check() && Auth::user()->is_admin)
                            @can('Crear Noticias')
                                <div class="card-header pb-0 d-flex justify-content-between align-items-center w-100">
                                    <a href="{{ route('talleresnews.create') }}" class="btn btn-primary position-absolute" style="top: 15px; right: 15px; z-index: 1;">
                                    ...
                                    <div class="card h-100 shadow-sm text-start overflow-hidden" style="border-radius: 1rem;">
                                        ...
                                        @if(Auth::check() && Auth::user()->is_admin)
                                            <div class="position-absolute top-0 end-0 m-2 d-flex align-items-center gap-1 z-1">
                                                ...
                                                <a href="{{ route('talleresnews.edit', $noticias->id_noticia) }}" class="btn btn-sm btn-info px-2 py-1" title="Editar">
                                                    <i class="fas fa-pen-to-square"></i>
                                                </a>
                                                <form action="{{ route('talleresnews.destroy', $noticias->id_noticia) }}" method="POST" onsubmit="return confirm('¿Estás seguro de eliminar esta noticia?');">
                                                    @csrf
                                                    @method('DELETE')
                                        ...
                                        <a href="{{ route('talleresnews.show', $noticias->id_noticia) }}" class="text-decoration-none text-dark d-block w-100">
